...
class InstagramRecommenderReal:
    ...
    def get_real_recommendations(self, target_user, n=5):
        """Renvoie (posts_recommandés, utilisateurs_similaires) avec scoring RF."""
        ...
        scored_posts = {}
        for post, similarity_score in recommendations.items():
            post_fans = self.user_post_matrix[post][self.user_post_matrix[post] > 0]
            popularity = len(post_fans)

            sim_to_fans = 0.0
            if popularity > 0:
                user_vec = self.user_post_matrix.loc[target_user].values.reshape(1, -1)
                sims = []
                for fan in post_fans.index:
                    if fan in self.users:
                        fan_vec = self.user_post_matrix.loc[fan].values.reshape(1, -1)
            ...
            if self.rf_model is not None:
                features = np.array([[popularity, sim_to_fans]])
                rf_score = self.rf_model.predict_proba(features)[0][1]
                final_score = 0.6 * rf_score + 0.4 * similarity_score
            else:
                final_score = similarity_score

            scored_posts[post] = final_score

        sorted_posts = sorted(
            scored_posts.items(), key=lambda x: x[1], reverse=True
        )[:n]
...
class InstagramRealApp:
    ...
    def setup_ui(self):
        ...
        left_frame = tk.Frame(content, bg="#FFD9BE", width=260)
    ...
    def display_real_posts(self, target_user, top_posts, similar_users):
        ...
        for i, (post_name, score) in enumerate(top_posts, start=1):
            card = tk.Frame(self.scrollable_frame, bg="#EF9C82", relief="solid", bd=1)
